...
namespace BONE_GRAPHICS
{
	...
	void etuImage::Render()
	{
		...
		D3DXMatrixTransformation2D(&m_TransformMatrix, &D3DXVECTOR2(m_vec3Position.x, m_vec3Position.y), 0, &D3DXVECTOR2(m_fScaleX, m_fScaleY), 0, rotAngle, &D3DXVECTOR2(0, 0));

		m_pSprite->SetTransform(&m_TransformMatrix);

		m_pSprite->Begin(D3DXSPRITE_ALPHABLEND | D3DXSPRITE_SORT_TEXTURE);

		m_pSprite->Draw(m_pTextures, &m_rImageRect, 0, &(m_vec3ParentPosition + m_vec3Position), 0xffffffff);
	...
	{
		D3DXMatrixIdentity(&m_TransformMatrix);
		D3DXMatrixTransformation2D(&m_TransformMatrix, 0, 0, &D3DXVECTOR2(m_fScaleX, m_fScaleY), &D3DXVECTOR2(m_vec3Position.x + 42, m_vec3Position.y + 46), rotAngle, 0);
